...
To document a security concern, communication is facilitated with the project curators:
  ...
  - **Method**: Reports are submitted via the repository’s [GitHub Issues](https://github.com/Amey-Thakur/DIGITAL-BOOKSTORE/issues) interface to formally record security-related findings.
...
## Implementation Context: Full-Stack Web Architecture

This project is implemented using the LAMP stack (PHP 7.4+ and MySQL), as defined by the project’s technical scope.

- **Execution Environment**: Execution occurs within controlled server environments or local development stacks such as XAMPP, Apache, or MariaDB.
- **Scope Limitation**: This policy applies exclusively to the source code and academic materials contained within this repository and does not extend to external frameworks, third-party libraries, or server-level security configurations.
